    ...
    <div v-if="canManageTwoFactor" class="space-y-6">
        ...
        >
            <p class="text-sm text-muted-foreground">
                Saat mengaktifkan autentikasi dua faktor, Anda akan diminta PIN
                aman saat masuk. PIN ini dapat diperoleh dari aplikasi yang
                mendukung TOTP di ponsel Anda.
            </p>

            <div>
                <Button v-if="hasSetupData" @click="showSetupModal = true">
                    <ShieldCheck />{{ t('common.continue') }} pengaturan
                </Button>
                <Form
                    v-else
                    v-bind="enable.form()"
        ...
        <div v-else class="flex flex-col items-start justify-start space-y-4">
            <p class="text-sm text-muted-foreground">
                Anda akan diminta PIN acak yang aman saat masuk, yang dapat
